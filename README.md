...
- Ride creation & matching:
  ...
  - **Offline rides** (driver generates pairing code, ride logged after trip)
...
- **Offline rides**
  - Driver generates an **offline pairing code** (short-lived, stored in DB)
  - Rider (or call center) creates an offline ride with that code
  - Backend validates pairing, marks the code as used, and records the ride as:
    - `type = OFFLINE`
...
- **Models** (`src/model`)
  - Sequelize model definitions for `User`, `Driver`, `Ride`, `OfflinePairing`, etc.
...
- **Infrastructure**:
  - MySQL database (users, drivers, rides, pairing codes)
